...
def linearlization(data):
    num_image, num_channels,num_row, num_col, num_frame = data.shape
    data_out = np.ones((num_image,num_channels, num_row*num_col, num_frame)).astype('uint8')
    for i in tqdm(range(num_image)):
        for f in range(num_frame):
            for c in range(num_channels):
                data_out[i,c,:,f] = data[i,c,:,:,f].flatten()
    return data_out
